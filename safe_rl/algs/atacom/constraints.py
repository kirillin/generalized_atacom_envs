...
class StateConstraint:
    ...
    def fun(self, q, x=None, origin_constr=False):
        ...
        if origin_constr:
            ...
            if self.slack_type is None:
                ...
                ret = self.c_last + np.exp(self.beta * self.s)
    ...
    def jac_slack(self):
        if self.slack_type is None:
            ...
            return self.beta * np.exp(self.beta * self.s)
        elif self.slack_type == 'softplus':
    ...
    def reset_slack(self, q, x=None):
        c_init = np.maximum(-self.fun(q, x, origin_constr=True), self.threshold)
        if self.slack_type is None:
            self.s = np.array([])
        elif self.slack_type == 'square':
            self.s = np.sqrt(c_init / self.beta)
        elif self.slack_type == 'exp':
            self.s = np.log(c_init) / self.beta
        elif self.slack_type == 'softplus':
            ''' self.s is exp(beta * s) '''
